...
Verite's initial release contains only the USA KYC/AML process definition ([https://verite.id/definitions/processes/kycaml/0.0.1/usa](https://verite.id/definitions/processes/kycaml/0.0.1/usa)). Additional process definitions will be developed based on existing practices/requirements per country.
