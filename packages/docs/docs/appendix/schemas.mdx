...
Verite's intial release contains only the USA KYC/AML process definition ("https://verite.id/schemas/definitions/1.0.0/kycaml/usa",). Additional process defnitions will be developed based on existing practices/requirements per country.
