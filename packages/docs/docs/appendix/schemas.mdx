...
- Support for additional use cases (such as jurisdiction shopping) that require additional data should be accomplished in ways that enable the subject to provide the least amount of data required. This may be accomplished with ZKPs or by issuing separate credentials containing the additional attributes (such as residence), enabling wallet aggregation of minimal required data.

Verite's initial release contains only the USA KYC/AML process definition ([https://verite.id/schemas/definitions/1.0.0/kycaml/usa](https://verite.id/schemas/definitions/1.0.0/kycaml/usa)). Additional process definitions will be developed based on existing practices/requirements per country.
...
[See further discussion of registry forensics flows](/docs/patterns/smart-contract-verite#auditing-and-forensics-sequences)
