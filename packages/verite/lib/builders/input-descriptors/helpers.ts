...
export const REQUIRED_PREDICATE: InputDescriptorConstraintDirective = InputDescriptorConstraintDirective.REQUIRED

const SubjectFieldId = "subjectId"


export const stringValueConstraint = (property: string, prefix?: string) => (b: InputDescriptorConstraintFieldBuilder): void => {
...
export const minimumValueConstraint = (property: string, minValue: number, prefix?: string) =>(b: InputDescriptorConstraintFieldBuilder): void => {
  const path = `${prefix ?? ''}${property}`

  b.path(attestationConstraintPaths(path))
    .purpose(`We can only accept credentials where the ${property} value is above ${minValue}.`)
    .predicate(REQUIRED_PREDICATE)
    .filter({
      type: "number",
      minimum: minValue
...
export const schemaConstraint = (schema: string) => (b: InputDescriptorConstraintFieldBuilder) : void => {
  if (schema) {
  b.id(CREDENTIAL_SCHEMA_PROPERTY_NAME)
    .path(vcConstraintPaths(`${CREDENTIAL_SCHEMA_PROPERTY_NAME}.${ID_PROPERTY_NAME}`))
    .filter({
        ...STRING_SCHEMA,
        pattern: schema
      })
...
export const subjectContraint = (b: InputDescriptorConstraintFieldBuilder) : void => {
  b.id(SubjectFieldId)
